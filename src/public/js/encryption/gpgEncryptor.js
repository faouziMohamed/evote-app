...
class GPGEncryptor {
   ...
   *  */
  static async generateKeysAsymetric(
    userIDs = [{ name: '', email: '' }],
    passphrase = '',
  ) {
  ...
  static async fromArmoredKeys({
    ...
    GPGEncryptor.#typeAndContentVerification(publicKeyArmored, 'Public key');
    GPGEncryptor.#typeAndContentVerification(privateKeyArmored, 'Private key');
    await decryptKey({
    ...
    that.#privateKey = privateKeyArmored;
    that.#publicKey = publicKeyArmored;
    that.#instance = that;
    return that;
  }
